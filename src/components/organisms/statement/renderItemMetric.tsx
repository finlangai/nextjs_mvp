...
const renderItemMetric = (item: FinancialMetric) => {
    return (
        <React.Fragment key={item.name}>
            <div className='border-b border-fintown-lnr-1 '>
                <div className='flex justify-between py-[18px] items-center hover:bg-fintown-hvr-btn-1 px-[16px]'>
                    <div className="flex flex-grow-0 flex-shrink-0 basis-[350px] items-center pr-[30px]">
                        <div className="flex flex-nowrap flex-grow flex-shrink basis-auto">
                            <div className='text-sm text-fintown-txt-1'>
                                {item.name}
                    ...
                    <div className="flex-grow flex-shrink basis-auto ">
                        <div className='flex justify-between'>
                            {item.values.slice(0, 9).map((val) => (
                                <div className="text-xm text-fintown-txt-1 min-w-[calc(11.1111%)] pl-[16px]" key={`${val.period}-${val.year}-${val.quarter}`}>
